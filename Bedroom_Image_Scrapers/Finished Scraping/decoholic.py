...
def get_card_links(link_par):
    ...
    for i in trange(1, 13):
        ...
        a_tags = driver.find_elements(By.XPATH, "//article/h2/a")
        for tag in a_tags:
            try:
...
def get_image_links(card_links):
    ...
    for link in card_links:
        ...
        driver.implicitly_wait(4)
        time.sleep(0.2)
        img_tags = driver.find_elements(By.CSS_SELECTOR, ".wp-block-image > img")
        if len(img_tags) == 0:
            img_tags = driver.find_elements(By.CSS_SELECTOR, ".separator > img")
        if len(img_tags) == 0:
            img_tags = driver.find_elements(By.CSS_SELECTOR, "p > a > img")
        for tag in tqdm(img_tags):
            driver.execute_script("arguments[0].scrollIntoView();", tag)
            time.sleep(2)
        try:
            title = driver.find_element(By.XPATH, "//*[@id='tm-content']/article/div[1]/div[2]/h1").get_attribute("innerHTML")
        except:
            title = link.split("/")[3]
        img_links_string = ""
        for tag in img_tags:
...
# LINK = "https://decoholic.org/category/bedrooms/page/"
# card_links = get_card_links(link_par=LINK)
# df = pd.DataFrame(card_links, index=None, columns=["card_links"])
# df.to_csv("decoholic_cards.csv", index=False)
...
df1 = pd.read_csv("decoholic_cards.csv", encoding='ISO-8859-1')
card_links = df1["card_links"].to_list()
try:
    df2 = pd.read_csv("decoholic.csv", encoding='ISO-8859-1')
...
while(end_idx < final_entry):
    ...
    write_csv(card_links[0:end_idx], img_links, img_titles, "decoholic.csv")
    str_idx = end_idx
    ...
    write_number_of_items_done(str_idx, "decoholic.txt")
driver.close()
pbar.close()
