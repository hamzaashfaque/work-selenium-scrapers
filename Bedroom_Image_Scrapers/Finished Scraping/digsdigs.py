...
def get_card_links(link_par):
    ...
    for i in trange(1, 18):
        ...
        a_tags = driver.find_elements(By.XPATH, "//main//a[@rel='bookmark']")
        for tag in a_tags:
            try:
...
def get_image_links(card_links):
    ...
    for link in card_links:
        try:
            ...
            continue
        driver.implicitly_wait(4)
        img_tags = driver.find_elements(By.XPATH, "//*[@class='entry-content']//img")
        for tag in tqdm(img_tags):
            driver.execute_script("arguments[0].scrollIntoView();", tag)
            time.sleep(0.5)
        img_tags = driver.find_elements(By.XPATH, "//*[@class='entry-content']//img")
        title = driver.find_element(By.CLASS_NAME, "entry-title").get_attribute("innerHTML")
        img_links_string = ""
        for tag in img_tags:
...
LINK = "https://www.digsdigs.com/category/decorating/bedroom-designs/page/"
#card_links = get_card_links(link_par=LINK)
#write_csv_card_links(card_links=card_links, filename="digsdigs_cards.csv")
...
df1 = pd.read_csv("digsdigs_cards.csv", encoding='ISO-8859-1')
card_links = df1["card_link"].to_list()
try:
    df2 = pd.read_csv("digsdigs.csv", encoding='ISO-8859-1')
...
while(end_idx < final_entry):
    ...
    write_csv(card_links[0:end_idx], img_links, img_titles, "digsdigs.csv")
    str_idx = end_idx
    ...
    write_number_of_items_done(str_idx, "digsdigs_num.txt")
driver.close()
pbar.close()
