...
def get_image_links(card_links):
    ...
    for card in tqdm(card_links):
        ...
        for tag in tqdm(tags):
            ...
            time.sleep(1)
        title = driver.find_element(By.CLASS_NAME, "article-heading").get_attribute("innerHTML").replace("\n", " ").strip()
        print("\n" + title)
        tags = driver.find_elements(By.CSS_SELECTOR, ".figure-media > div > img")
        try:
...
# LINK = "https://www.mydomaine.com/bedroom-4628433"
# card_links = get_card_links(link_par=LINK)
# write_csv_card_links(card_links=card_links, filename="mydomaine_links.csv")

df1 = pd.read_csv("mydomaine_links.csv", encoding="ISO-8859-1")
card_links = df1["card_link"].to_list()
img_url, img_titles = get_image_links(card_links)
write_csv(card_links, img_url, img_titles, "mydomaine.csv")